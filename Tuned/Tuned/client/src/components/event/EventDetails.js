...
const EventDetails = (props) => {
  ...
  const { userEvents, addUserEvent, deleteUserEvent } = useContext(
    UserEventContext
  );
...

  {
    userEvents.forEach((rel) => {
      const foundUser = users.filter((singleUser) => {
        return (
          rel.applicationUser.id === singleUser.id && rel.eventId === event.id
        );
      })[0];
  ...
  const constructNewUserEvent = (currentEvent) => {
    const alreadyAddedEventRel = userEvents.find(
      (userEvent) =>
        userEvent.eventId === currentEvent.id &&
        userEvent.applicationUser.id === user.id
    );
  ...
  useEffect(() => {
    ...
    if (event.location) {
      fetch(
        `https://maps.googleapis.com/maps/api/geocode/json?address=${event.location}&key=${process.env.REACT_APP_GOOGLE_API_KEY}`
      )
        .then((res) => res.json())
        .then((res) => {
  ...
  return (
    <section className="event top-space">
              ...
              <h3 className="event__date">{event.date ? event.date.substring(0, 10) : ""}</h3>
      ...
      <section className="eventLocationDate">
      ...
      >
        {addedUserEventMode ? "Add" : "Remove"}
      </button>

      <div className="event_admin_user">
        {eventUser.adminUser ? `Admin: ${eventUser.username}` : ""}
      </div>
